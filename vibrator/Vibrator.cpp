...
/*
 * Index must be written after duration has been set for effects to work.
 * Hence we map the nodes and return to activate the hal.
 */
ndk::ScopedAStatus Vibrator::mapNodes(uint32_t timeMs, uint32_t index) {
    write_haptic_node(duration_node, timeMs);
    write_haptic_node(index_node, index);
    return on(timeMs, nullptr);
}
